...
SF_Data <- subset( SF_Data, select = -c(1:3, 5:18,23:24, 26:27))

# Rename column names (currently column names begin in row 2)
names(SF_Data)[1] <- "School Code"
names(SF_Data)[2] <- "Free and Reduced Meal Program"
names(SF_Data)[3] <- "Foster"
names(SF_Data)[4] <- "Homeless"
names(SF_Data)[5] <- "Migrant Program"
names(SF_Data)[6] <- "English  Learner  (EL)"
```

```{r}
# Remove rows 1 and 2
Demographic_Data <- SF_Data[-1, ] # To remove the 1st row
Demographic_Data <- SF_Data[-1, ] # To remove the 2nd row
#SF_Data is now ready to merge
```
